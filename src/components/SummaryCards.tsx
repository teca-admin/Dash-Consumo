...
export function StatCard({ title, value, icon: Icon, colorClass, onClick }: StatCardProps) {
  return (
    <Card 
      className={cn(
        "shadow-sm hover:shadow-md transition-all h-full flex flex-col justify-center",
    ...
    >
      <CardContent className="p-4 flex items-center justify-between">
        <div>
          ...
          <h3 className="text-xl font-bold mt-0.5 text-slate-800">{value}</h3>
        </div>
...
export function ProviderCard({ name, value, colorClass, onClick }: ProviderCardProps) {
  return (
    <Card 
      onClick={onClick}
      className={cn(
        "shadow-sm hover:shadow-md transition-all overflow-hidden relative cursor-pointer active:scale-95 h-full",
        colorClass
      )}
    >
      <CardContent className="p-4">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">{name}</p>
            <p className="text-xs font-medium mt-0.5 opacity-90">Valor da NF</p>
            <h3 className="text-lg font-bold mt-0.5">{value}</h3>
          </div>
          <div className="bg-white/10 p-1.5 rounded-lg backdrop-blur-sm">
            <span className="text-[10px] font-bold">{name.substring(0, 2).toUpperCase()}</span>
          </div>
        </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      {providerStats.map((p, idx) => (
        <ProviderCard 
          ...
          colorClass={idx === 0 ? "bg-gradient-to-br from-slate-800 to-slate-900 text-white" : "bg-gradient-to-br from-purple-800 to-purple-900 text-white"}
